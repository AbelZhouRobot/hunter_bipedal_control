...
namespace legged
{
  bool LeggedController::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &controller_nh)
  {
    ...
    /*
    PinocchioInterface:  构造的ocs2数据与pinocchio的模型接口，
    CentroidalModelPinocchioMapping ：
        用于质心动力学模型和Pinocchio库之间映射。这个类将质心动力学的状态和输入转换为Pinocchio关节空间的状态和速度，并提供了计算雅可比矩阵的方法。
    endEffectorIds： 接触点frame的id，初始化一个《末端运动学计算接口》
    */
