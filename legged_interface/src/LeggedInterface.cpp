...
void LeggedInterface::setupOptimalControlProblem(const std::string &taskFile, const std::string &urdfFile,
  ...
  std::cout << "\n#######################\n"
            << "\n###### : setupOptimalControlProblem conduct in: " << std::endl;
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> LeggedInterface::getSoftSwingTrajConstraint(
    const EndEffectorKinematics<scalar_t> &eeKinematics, size_t contactPointIndex, const std::string &taskFile,
    bool verbose) {
  scalar_t weight = 10;
...
matrix_t LeggedInterface::initializeInputCostWeight(const std::string &taskFile, const CentroidalModelInfo &info) {
  ...
  std::cout << " info.numThreeDofContacts is: \n " << info.numThreeDofContacts << std::endl;
  std::cout << " info.numSixDofContacts   is: \t " << info.numSixDofContacts << std::endl;
  std::cout << " rTaskspace is: \n " << rTaskspace.topLeftCorner(totalContactDim, totalContactDim) << std::endl;
...
std::unique_ptr<EndEffectorKinematics<scalar_t>> LeggedInterface::getEeKinematicsPtr(
    const std::vector<std::string> &footNames, const std::string &modelName) {
